...
import { pricing, pricingSummary } from "./site.config";

// Shared customer-facing chat AI — used by the public ChatWidget (/api/chat)
// and the inquiry live-chat (/api/inquiry). Falls back to a keyword responder
...
/** Generate a reply for the given conversation. Uses Claude when configured,
 *  otherwise the keyword fallback. `extraContext` is prepended to the system
 *  prompt (e.g. the inquiry's lead details). */
export async function chatReply(messages: ChatMessage[], extraContext?: string): Promise<string> {
  const lastUser = [...messages].reverse().find((m) => m.role === "user");
  if (process.env.ANTHROPIC_API_KEY && messages.length > 0) {
    try {
  ...
  return ruleBasedReply(lastUser?.content ?? "");
}
